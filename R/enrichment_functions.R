...
get_interaction_from_ORA <- function(query, 
    ...
    if (is(query, "list")) {
        query <- lapply(query, 
                        function(x) check_vector_char(x, 
                                                      var.name = "'query '"))
    ...
    # check organism
    organism = check_vector_char(X = organism, 
                                 X.length = 1, 
                                 default = "hsapiens", 
                                 var.name = "'organism' ")
    ...
    if (is(query, "list")) {
        ...
        for (i in names(query)) {
            ...
            res.graph[[i]] <- igraph::graph_from_data_frame(term_map[[i]], 
                                                            directed = FALSE)
            res.graph[[i]] <- set_vertex_attr(
                graph = res.graph[[i]], 
                name = "mode", index = term_map[[i]]$input,
                value = "core"
            )
            res.graph[[i]] <- set_vertex_attr(
                graph = res.graph[[i]], 
                name = "mode", index = term_map[[i]]$target,
                value = "extended"
            )
        ...
        res.graph <- set_vertex_attr(graph = res.graph, 
                                     name = "mode", 
...
get_ORA <- function(query, sources = NULL, organism = "hsapiens") {
    ...
    if (is(query, "list")) {
        ...
        for (i in names(query)) {
            ORA <- gprofiler2::gost(
                query = query[[i]], organism = organism, 
                significant = FALSE, 
                sources = sources,
                multi_query = FALSE
            )
...
get_go_info <- function(go) {
    res <- AnnotationDbi::select(x = GO.db, 
                                 keys = go, 
                                 keytype = "GOID", 
