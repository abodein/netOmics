...
#' data(hmp_T2D)
#' # grn only on gene
#' cluster.mRNA <- timeOmics::getCluster(hmp_T2D$getCluster.res, user.block = "RNA")
#' X <- hmp_T2D$raw$RNA
#' grn.res <- get_grn(X = hmp_T2D$raw$RNA, cluster = cluster.mRNA, method = "aracne")
#' 
#' 
...
#' @export
get_grn <- function(X, cluster=NULL, method = c("aracne"), type = "gene"){
    ...
    if(is.null(cluster)){ # no clusteing info -> perform grn on all molecules
        ...
        grn.graph <- igraph::set_vertex_attr(graph = grn.graph, name = "type", value = type)
        ...
        for(i in names(mol_cluster)){
            ...
            grn.graph.cluster <- igraph::set_vertex_attr(graph = grn.graph.cluster, name = "type", value = type)
