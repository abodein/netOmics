...
In the longitudinal context of the data, kinetic cluster subnetworks are built 
...
Interaction).
This time, any inference is performed. Instead, known interactions are extracted
...
degree neighbours can also be collected (option `user.ego = TRUE`)
...
(or list of network) in `graph2`, based on the shared verticees between
the networks.
...
We can use basics graph statistics to explore the network such as degree 
...
RWR is a powerful tool to explore the MO networks which simulates a particule 
